...
void CalculTab(std::vector<int>& tab, int debutSeg, int nbData)
{
  for(int i = debutSeg; i < nbData; i+=2)
...
int main(int argc, char ** argv)
{
    ...
    std::thread thread1(CalculTab, std::ref(data), 0, nbData);
    std::thread thread2(CalculTab, std::ref(data), 1, nbData);
